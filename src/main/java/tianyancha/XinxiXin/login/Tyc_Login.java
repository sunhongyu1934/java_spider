package tianyancha.XinxiXin.login;
...
public class Tyc_Login {
    private static RedisClu redisClu=new RedisClu();
    public Object login(String user,String passwd) throws IOException, InterruptedException {
        ...
        while (true) {
            try {
                while (true) {
                    ip=redisClu.getZsetByKey("ip","0","0").iterator().next();
                    if(Dup.nullor(ip)){
                        break;
                    }else{
                        Thread.sleep(500);
                ...
                driver.findElement(By.xpath("//*[@id=\"web-content\"]/div/div[2]/div/div[2]/div/div[3]/div[2]/div[2]/input")).sendKeys(user);
                driver.findElement(By.xpath("//*[@id=\"web-content\"]/div/div[2]/div/div[2]/div/div[3]/div[2]/div[3]/input")).sendKeys(passwd);
                Thread.sleep(1000);
                ...
                int b=imgduibi.findXDiffRectangeOfTwoImage("/data1/java_spider/jiyan/sy/"+user+"tt.jpg", "/data1/java_spider/jiyan/sy/"+user+"tt2.jpg");
                int a=b-7;
                //int f=(a*7)/8;
                ...
                if(!doc.outerHtml().contains("请输入手机号")&&!doc.outerHtml().contains("手机号或密码错误")) {
                    ...
                    return mapcookie;
                }else if(doc.outerHtml().contains("手机号或密码错误")||doc.outerHtml().contains("请确认输入正确的手机号码和密码")||doc.outerHtml().contains("账户存在风险")){
    ...
    public static void tt(WebDriver driver,String username) throws Exception {
        ...
        for(int qw=0;qw<52;qw++) {
            FileUtils.copyFile(file, new File("/data1/java_spider/jiyan/ml/"+username+qw+".png"));
            ar.add("/data1/java_spider/jiyan/ml/"+username+qw+".png");
        }
        System.out.println("下载完毕，开始拼接");
        System.out.println(imgduibi.combineImages(ar, aa, 26, 10, 58, "/data1/java_spider/jiyan/sy/"+username+"tt.jpg", "jpg"));
        driver.close();
        driver.switchTo().window(handle);
    }

    public static void tt2(WebDriver driver,String username) throws Exception {
        List ar=new ArrayList();
        Document doc= Jsoup.parse(driver.getPageSource());
        ...
        File file=captureElement(driver.findElement(By.xpath("/html/body/img")));
        for(int qw=0;qw<52;qw++) {
            FileUtils.copyFile(file, new File("/data1/java_spider/jiyan/ml2/"+username+qw+".png"));
            ar.add("/data1/java_spider/jiyan/ml2/"+username+qw+".png");
        }
        System.out.println("下载完毕，开始拼接");
        System.out.println(imgduibi.combineImages(ar, aa, 26, 10, 58, "/data1/java_spider/jiyan/sy/"+username+"tt2.jpg", "jpg"));
        driver.close();
        driver.switchTo().window(handle);
    }
