...
public class tyc_guoxin {
    ...
    public static void duqu(Connection con,Key k) throws SQLException, InterruptedException {
        String sql="select se_url from tyc_seurl_car";
    ...
    public static void serachget(Key k,Connection con) throws IOException, InterruptedException, SQLException {
        String sql="insert into comp_name_car(quan_chemg,t_id) values(?,?)";
        PreparedStatement ps=con.prepareStatement(sql);
        Document doc;
        while (true) {
            try {
                ...
                for(int a=1;a<=5;a++) {
                    doc = detailget(key.replace("?key=%E6%B1%BD%E8%BD%A6","/p"+a+"?key=%E6%B1%BD%E8%BD%A6"));
                    Elements eles = getElements(doc, "div.search_result_single.search-2017.pb25.pt25.pl30.pr30 div.search_right_item");
                    int j=0;
                    if (eles != null) {
                        for (Element e : eles) {
                            ...
                            ps.setString(2,url.replace("https://www.tianyancha.com/company/", ""));
                            ps.executeUpdate();
    ...
    public static void data(Url u, Connection con, String[] zhua, Key k) throws InterruptedException, IOException, SQLException {
        Tyc_quan_guo t=new Tyc_quan_guo(con,zhua);
