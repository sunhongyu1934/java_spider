...
;define("search-result/modules/search-bar/main", ["require", "exports", "module", "search-result/modules/common/js/config", "common/widgets/plat/exposure"], function(require) {
    ...
    $.widget("custom.catcomplete", $.ui.autocomplete, {
        ...
        _renderMenu: function(ul, a) {
            function c(a, c) {
                ...
                for (var i = 0, l = c.length; l > i; i++)
                    h._renderItemData(ul, c[i])
            }
            var h = this;
            a = a[0],
    ...
    C.catcomplete({
        ...
        source: function(a, c) {
            "" != $.trim(a.term) ? $.lgAjax({
                ...
                success: function(a) {
                    if (a)
                        if (a.POSITION && a.POSITION.length || a.COMPANY && a.COMPANY.length) {
                            var h = [];
                            h.push(a),
                                c(h)
                        } else
        ...
        select: function(a, ui) {
            return global.searchTj.suginput = C.val(),
                C.val(ui.item.cont),
        ...
        $("#submit").on("click", function() {
            var a = C.val()
                , c = /[\\\/]/g;
            if (a = a.replace(c, " "),
            "" != global.queryParam.city) {
                var h = "city=" + global.queryParam.city;
            ...
            "1" == global.isSchoolJob && (h += "&isSchoolJob=1"),
                ...
                $.lgAjax({
                    ...
                    window.location.href = GLOBAL_DOMAIN.ctx + "/jobs/list_" + a + "?" + h
                }).fail(function() {
                    console.log("error")
                }),
    ...
    var I = require("common/widgets/plat/exposure").postoA
});
...
;define("common/components/util/emitter", ["require"], function() {
    function a() {}
    var h = a.prototype;
    return h._getEvents = function() {
        ...
        h.on = function(a, h) {
            ...
            var g = c[a].length;
            if (g >= v && 0 !== v)
                throw new RangeError("Warning: possible Emitter memory leak detected. " + g + " listeners added.");
            return c[a].push(h),
                this
        }
        ,
                    ...
                    if ("PCP" == a.content.data.markLocation && a.content.data[g].length > 0) {
                        if ($(".item_con_list li").length > 0) {
                            ...
                            $(".item_con_list li").each(function() {
                                $(this).find(".item_salary").before(_.clone())
                            }),
                        ...
                        var b = $('<i class="company_icon_dsz"></i>');
                        $(".job-name .name").append(b.clone()),
                            $(".company_icon_dsz").css("backgroundImage", "url(" + c + (a.content.data[g][1] || a.content.data[g][0]) + ")")
                    } else if ("PPF" == a.content.data.markLocation)
                        for (var h = (g.split(","),
                            $(".my_collections li")), _ = $("<i class='pos_icon'></i>"), i = 0; i < h.length; i++) {
                            var v = h.eq(i).data("company");
                            a.content.data[v] && a.content.data[v].length > 0 && (h.eq(i).find(".co_time").before(_.clone()),
                                h.eq(i).find(".pos_icon").css("backgroundImage", "url(" + c + (a.content.data[v][1] || a.content.data[v][0]) + ")"))
                        }
                    else if ("PPR" == a.content.data.markLocation)
                        for (var h = (g.split(","),
                            $(".rec_pos_item")), _ = $("<i class='pos_icon'></i>"), i = 0; i < h.length; i++) {
                            var v = h.eq(i).data("company");
                            a.content.data[v] && a.content.data[v].length > 0 && (h.eq(i).find(".coName").after(_.clone()),
                                h.eq(i).find(".pos_icon").css("backgroundImage", "url(" + c + (a.content.data[v][1] || a.content.data[v][0]) + ")"))
                        }
                    else if ("PPQ" == a.content.data.markLocation)
                        for (var h = (g.split(","),
                            $(".con_list_item")), _ = $("<i class='pos_icon'></i>"), i = 0; i < h.length; i++) {
                            var v = h.eq(i).data("companyid");
                            a.content.data[v] && a.content.data[v].length > 0 && (h.eq(i).find(".p_top").append(_.clone()),
                                h.eq(i).find(".pos_icon").css("backgroundImage", "url(" + c + (a.content.data[v][1] || a.content.data[v][0]) + ")")),
...
;define("search-result/modules/positions/main", ["require", "exports", "module", "common/widgets/plat/exposure", "search-result/modules/common/js/config", "common/components/util/emitter", "common/components/pager/main", "common/components/list-base/main", "common/widgets/plat/addIcon"], function(require) {
    function a(a) {
        if (g(),
            C($("#footer").hasClass("footer_fix") ? 68 : 0),
            1 == a ? j("jids") : j("jids", a),
            S(),
        "false" == E) {
                ...
                success: function(a) {
                    if (1 == a.state) {
                        ...
                        for (var i = 0; i < _.length; i++) {
                            var y = $(_[i]).data("companyid");
                            if (1 == g[y]) {
                                var C = $(_[i]).find(".company_name a");
                                C.after(h.clone())
                            }
                        }
                        c()
    ...
    function y(a, c) {
        ...
        for (var y = Q.data, i = y.length - 1; i >= 0; i--)
            y[i].curpage = c;
        Q.data.hrInfoMap = g || [],
            b(),
            v(),
        Q.firstPage && Q.totalCount > Q.pageSize && (w(),
            I())
    }
    function C(a) {
    ...
    function b() {
        var a = $("#s_position_list input[name=abtCode]").val().trim()
            , c = new T({
            emitter: O,
            container: U.find(".item_con_list"),
    ...
    function w() {
        var a = new z({
            emitter: O,
            container: U.find(".item_con_pager"),
        ...
        a.prototype = {
            ...
            init: function() {
                ...
                this.elePrev = a.find(".prev"),
                    ...
                    this.pagerContainer = U.find(".item_con_pager"),
                    this.initEvent()
            },
            initEvent: function() {
                var a = this;
    ...
    function j(a, c) {
        ...
        _.each(function(a, c) {
            if ($(c).attr("data-lg-tj-id")) {
                ...
                b && "|" != b ? (g.push([_, y, C]),
                    v.push(b)) : h.push([_, y, C])
            }
        }),
        g.length > 0 && v.length > 0 && L(g, "p", v),
        h.length > 0 && L(h, "p");
    ...
    function S() {
        for (var a = [], c = $(".con_list_item"), i = 0; i < c.length; i++)
            a.push(c.eq(i).data("companyid"));
        a = P(a);
        var h = a.join(",")
            , g = require("common/widgets/plat/addIcon").loadIconInfo;
        g({
            companyIds: h,
            markLocation: "PPQ"
        }, h)
    }
    var L = require("common/widgets/plat/exposure").exposure;
    postoA = require("common/widgets/plat/exposure").postoA;
    var M = require("search-result/modules/common/js/config")
        , A = require("common/components/util/emitter")
        , O = new A
        , z = require("common/components/pager/main")
        , T = require("common/components/list-base/main")
        , E = $("#isSEO").val()
        , R = $("#pageNoSEO").val()
        , B = $("#totalPageCountSEO").val()
        , D = $("#resultLengthSEO").val()
        , G = $("#filterOption").val()
        , U = $("#s_position_list")
        , Q = {
        firstPage: !0,
    ...
    "false" == E ? F.getList(H) : (1 != R && (H.first = !1,
        Q.firstPage = !1),
        H.pn = parseInt(R),
        H.kd = global.keyword,
    0 == D && global.needAddtionalResult && $(".no_position_wrapper").hide(),
        v(),
        a(parseInt(R)),
    "2" == G && N(500)),
        O.on("pager:go", function(a) {
            H.first = !1,
                H.pn = a,
                H.kd = global.keyword,
                Q.pn = a,
        ...
        $(".pager_container a, #order .page a").on("click", function(e) {
            e.preventDefault(),
                "1" == G || $("#filterCollapse").hasClass("collapsed") ? window.location.href = $(this).attr("href") + "?filterOption=2" : ("2" == G || "3" == G) && (window.location.href = $(this).attr("href") + "?filterOption=3")
        }),
...
;define("search-result/modules/filter/main", ["require", "exports", "module"], function() {
    ...
    function c(a) {
        ...
        3 == filterOption && v(),
            ...
            c.attr("title", "点击收起筛选项")) : (w = g.height(),
            g.animate({
                opacity: 0,
                height: b
            }, 500, function() {
    ...
    function h() {
        ...
        $(".multi-chosen").each(function() {
            ...
            "行业领域：" == a && $(this).find(".chosen").each(function() {
                -1 == $.inArray($.trim($(this).text()), C) && C.push($.trim($(this).text()))
            }),
            ...
            L = $(".current-subway").text(),
            I = $(".current-route").text();
        var k = []
            , j = "1" === a("isSchoolJob", location.search)
            , B = -1 != $.inArray("应届毕业生", c);
        c.length > 0 && k.push(B || j ? "&gj=" : "&gj=" + c.join(",")),
        h.length > 0 && k.push("&xl=" + h.join(",")),
        y.length > 0 && k.push("&jd=" + y.join(",")),
        C.length > 0 && k.push("&hy=" + C.join(",")),
        v.length > 0 && k.push("&gm=" + v.join(",")),
            A = k.join(""),
        g && (_ += "&city=" + g),
        b && "不限" != b && (_ += "&district=" + b),
        w && "不限" != w && (_ += "&bizArea=" + w),
        L && (_ += "&subwayLine=" + L),
        I && "不限" != I && (_ += "&subwayStation=" + I),
            $("#order .order a").each(function() {
                $(this).hasClass("active") && ("默认" == $.trim($(this).text()) ? O += "px=default" : "最新" == $.trim($(this).text()) && (O += "px=new"))
            }),
        "不限" != $.trim($("#order .salary .text span").text()) && (O += "&yx=" + $.trim($("#order .salary .text span").text()));
        var N = $.trim($("#order .type .text span").text());
        B || -1 != $.inArray("应届", c) ? O += "&gx=全职" : j ? O += "&gx=" + c.join(",") : "不限" != N && (O += "&gx=" + N),
        (-1 != $.inArray("应届毕业生", c) || "实习" === N || j) && (A += "&isSchoolJob=1")
    }
    function v() {
        var a = 872 - $(".current-handle-position").width()
            , c = $(".current-handle-position").width();
    ...
    allHeight = g.height(),
        ...
        y.find(".btn-more").hover(function() {
            $(this).parent().addClass("active").next().addClass("unfolded")
        }, function() {
            $(this).parent().removeClass("active").next().removeClass("unfolded")
        }),
        y.find(".more").hover(function() {
            $(this).addClass("unfolded").prev().addClass("active")
        }, function() {
            $(this).removeClass("unfolded").prev().removeClass("active")
        }),
        y.find(".btn-more-hy").on("click", function() {
            $(this).parent().next().hasClass("unfolded") && $(this).parent().removeClass("active").next().removeClass("unfolded")
        }),
        $(document).on("click", function(e) {
            var a = e.target;
            if (0 == $(a).parents(".more-fields").length) {
                var c = y.find(".btn-more-hy");
                c.parent().next().hasClass("unfolded") && c.parent().removeClass("active").next().removeClass("unfolded")
            }
        }),
        y.find(".btn-more-hy").mouseenter(function() {
            $(this).parent().next().hasClass("unfolded") || $(this).parent().addClass("active").next().addClass("unfolded")
        }),
    "true" == global.isShowMoreIndustryField && y.find(".btn-more-hy").parent().addClass("active").next().addClass("unfolded"),
    ...
    $("#all_city").on("click", function() {
        ...
        if (h(),
            global.keyword)
            var c = GLOBAL_DOMAIN.ctx + "/jobs/allCity.html?keyword=" + encodeURIComponent(global.keyword) + "&" + O + A + _ + a;
        else
            var c = GLOBAL_DOMAIN.ctx + "/jobs/allCity.html?" + O + A + _ + a;
    ...
    h(),
        ...
        $(".hot-city-name, .more-city-name").on("click", function() {
            ...
            var a = $(this).text();
            if ("全国" == a)
                var c = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "#filterBox";
            else
                var c = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "#filterBox";
            window.location.href = c
        }),
        $('.detail-district-area, .choose-detail .tab-versus-content .contents[data-type="district"]').on("click", "a", function() {
            ...
            var a = $(".current_city").text()
                , c = $(this).text();
            if ("不限" == c)
                var v = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "#filterBox";
            else
                var v = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "&district=" + c + "#filterBox";
        ...
        $('.detail-area.detail-subway-area, .choose-detail .tab-versus-content .contents[data-type="subway"]').on("click", "a", function() {
            ...
            var a = $(".current_city").text()
                , c = $(this).text()
                , v = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "&subwayLine=" + c + "#filterBox";
            window.location.href = v
        }),
        $(".detail-bizArea-area").on("click", "a", function() {
            A = "",
                ...
                h();
            var a = $(".current_city").text()
                , c = $(".current_district").text()
                , v = $(this).text();
            if ("不限" == v)
                var y = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "&district=" + c + "#filterBox";
            else
                var y = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "&district=" + c + "&bizArea=" + v + "#filterBox";
        ...
        $(".detail-area.detail-route-area").on("click", "a", function() {
            A = "",
                ...
                h();
            var a = $(".current_city").text()
                , c = $(".current-subway").text()
                , v = $(this).text()
                , y = GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + "&city=" + a + "&subwayLine=" + c + ("不限" == v ? "" : "&subwayStation=" + v) + "#filterBox";
            window.location.href = y
        }),
        $(".multi-chosen").on("click", "a", function() {
            ...
            if ($(this).hasClass("chosen"))
                if ("行业领域：" == $(this).parent().find(".title").text() || $(this).parent().hasClass("other-hy")) {
                    ...
                    $(".hy-area a").each(function() {
                        $.trim($(this).text()) == c && ($(this).children(".delete").hide(),
            ...
            "不限" == $.trim($(this).text()))
                ...
                var c = $.trim($(this).text());
                $(".hy-area a").each(function() {
                    $.trim($(this).text()) == c && ($(this).addClass("chosen"),
            ...
            A = "",
                ...
                O = "",
                h(),
                a = $(".other-hy a.chosen").length > 0 ? GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + _ + "&isShowMoreIndustryField=true" : GLOBAL_DOMAIN.ctx + "/jobs/list_" + encodeURIComponent($("#keyword").val()) + "?" + O + A + _,
                a += "true" == global.needAddtionalResult ? "&needAddtionalResult=true#filterBox" : "#filterBox",
                window.location.href = a
        });
    var L = $(".tab-versus-content");
    $(".tab-versus-content .tabs li").on("mouseenter", function() {
        var a = $(this)
            , c = a.attr("data-type")
            , h = "-tab-active"
            , v = c + h
            , y = L.attr("class").split(/\s+/)
            , C = L.find('.tabs li[data-type="district"]').outerHeight(!1)
            , b = L.find('.tabs li[data-type="subway"]');
        y = y.filter(function(a) {
            return a.indexOf(h) < 0
        }),
            L.attr("class", y.join(" ") + " " + v),
            g.height("initial");
        var w = L.find('.contents[data-type="' + c + '"]').innerHeight();
        b.css("height", Math.max(w - C, C))
    }),
        g.on("click", ".toggle-anchor", function() {
            var a = $(this)
                , c = a.attr("data-toggle-type")
                , h = g.find(".toggle-anchor")
                , v = g.find(".toggle-target")
                , y = v.filter('[data-toggle-type="' + c + '"]');
            y.length > 0 ? a.hasClass("active") ? (a.removeClass("active"),
                y.hide()) : (h.removeClass("active"),
                v.hide(),
                a.addClass("active"),
                y.show()) : (y = g.find('.toggle-target[data-toggle-type="district-and-subway"]'),
    ...
    function c(a) {
        ...
        if (null === c || 0 === c.length)
            ...
            for (var g = "", i = 0; i < c.length; i++)
                g += '<li class="p_list_item"><div><a href="' + global.ctx + "/jobs/" + c[i].positionId + '.html" target="_blank" class="name" title="' + c[i].positionName + '">' + c[i].positionName + '</a><div class="salary">' + c[i].salary + '</div><div class="c_name" title="' + c[i].companyName + '">' + c[i].companyName + "</div></div></li>";
            I.removeClass("dn"),
