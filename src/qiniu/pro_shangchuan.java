...
public class pro_shangchuan {
    ...
    public static void data(Guan g) throws SQLException, InterruptedException {
        ...
        map.put("item_app_sum","app_pic");
        map.put("item_product_sum","company_logo");
        map.put("item_public_sum","logo");

        for(Map.Entry<String,String> entry:map.entrySet()){
            for(int x=0;x<=9;x++){
                String tablename=entry.getKey()+x;
                String field=entry.getValue();
                String sql="select source_y,tail_id,"+field+" from "+tablename+" where "+field+"!='' and "+field+" is not null";
                PreparedStatement ps=conn.prepareStatement(sql);
                ResultSet rs=ps.executeQuery();
                while (rs.next()){
                    try {
                        String logo=rs.getString(rs.findColumn(field));
                        String id=rs.getString(rs.findColumn("tail_id"));
    ...
    public static void chuli(Guan g) throws Exception {
        String sql="insert into project_logo_qiniu(tail_id,qi_logo,source_y,source_b) values(?,?,?,?)";
        PreparedStatement ps=conn.prepareStatement(sql);
